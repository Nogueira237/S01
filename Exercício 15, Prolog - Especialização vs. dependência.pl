...
especialista_deficiente(Aluno) :-
    ...
    D1 \= D2,  % são disciplinas diferentes

    % NÃO concluiu nenhuma disciplina de matematica
    \+ ( concluiu(Aluno, DM),
         disciplina(DM, _, matematica)
       ).

% Query: especialista_deficiente(X)
